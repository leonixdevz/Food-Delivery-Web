...
const getRestaurantFallback = (slug) => {
  if (!slug) return fallbackRestaurants;
  const match = fallbackRestaurants.find((restaurant) => restaurant.slug === slug);
  return match || null;
};

const mergeRestaurantCatalog = (restaurants = []) => {
  const catalog = Array.isArray(restaurants) ? [...restaurants] : [];
  fallbackRestaurants.forEach((fallbackRestaurant) => {
    const exists = catalog.some((restaurant) => restaurant.slug === fallbackRestaurant.slug);
    if (!exists) {
      catalog.push(fallbackRestaurant);
    }
  });
  return catalog;
};
...
router.get('/', asyncHandler(async (req, res) => {
  try {
    ...
    return res.json(mergeRestaurantCatalog(restaurants));
  } catch (error) {
    console.warn('Restaurant database unavailable, serving fallback list.', error.message);
    return res.json(fallbackRestaurants);
  }
}));
...
router.get('/:slug', asyncHandler(async (req, res) => {
  const { slug } = req.params;
    ...
    try {
      if (!connectDB.isConnected) {
        const fallbackRestaurant = getRestaurantFallback(slug);
        if (!fallbackRestaurant) {
          return res.status(404).json({ message: 'Restaurant not found.' });
        }
        return res.json(fallbackRestaurant);
      }
      ...
      console.warn(`Restaurant lookup failed for slug: ${slug}. Serving fallback data.`, error.message);
      const fallbackRestaurant = getRestaurantFallback(slug);
      if (!fallbackRestaurant) {
        return res.status(404).json({ message: 'Restaurant not found.' });
      }

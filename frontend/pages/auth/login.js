 ...
 *   - Redirecting after successful authentication or to onboarding for new signups
 *
...
const redirectDestination = new URLSearchParams(window.location.search).get('redirect') || '/pages/home/index.html';
 ...
 * @param {Object} payload - The request body (email, password).
...
authForm.addEventListener('submit', async (event) => {
  ...
  if (currentAuthMode === 'signin' && !isValidEmailAddress(email)) {
    return displayAuthFeedback('error', 'Please enter a valid email address.');
  }
  ...
  try {
    const requestPayload = { email: email.toLowerCase(), password };
    ...
    setTimeout(() => {
      window.location.href = currentAuthMode === 'signup' ? '/pages/auth/onboarding.html' : redirectDestination;
    }, 700);
  } catch (error) {
